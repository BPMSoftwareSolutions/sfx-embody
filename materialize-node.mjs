...
export async function materializeNode({ bundleFile, sdaRoot, outputRoot }) {
  ...
  for (const scenario of scenarios) {
    ...
    add(`${base}/evidence/authority.json`, pretty({ selection, selected, snapshotId: authority.snapshotId, projectionDigest: authority.projectionDigest,
      ...
      pinnedPlatformCommit: commit, resolver: { path: resolverFile, digest: resolverDigest, components: resolverComponents, status: 'CANDIDATE_PHYSICAL_PROVIDER' } }));
    ...
    add(`${base}/evidence/transformation-authority.json`, pretty(provider.lineage.filter(l => l.scenarioId === scenario.scenarioId).map(l => {
  ...
  for (const base of outputBases) {
    const file = one(files.filter(f => f.relativePath === base + '/evidence/mechanic-lineage.json'), 'LINEAGE_FILE');
    file.content = pretty(provider.lineage.filter(port => port.physicalFile.startsWith(base + '/body/')));
    file.digest = hash(file.content);
  }
