-- M3b: re-declare the equity provider-slot blueprint for the ten-operation
-- authority installed by add-equity-price-fallback-route.sql. The blueprint row
-- and its semantic object are reused; the ten-node definition supersedes the
-- five-node one (analysis.v_selected_semantic_definition selects the latest
-- definition per declared id, so the prior slots stop being the ones read).
-- Idempotent: (blueprint_pk, definition_digest) is the version key.
SET NOCOUNT ON;
SET XACT_ABORT ON;
BEGIN TRANSACTION;
...
DECLARE @blueprint bigint=(SELECT blueprint_pk FROM model.blueprint WHERE namespace_pk=@namespace AND blueprint_id=@blueprint_id);
IF @blueprint IS NULL THROW 51000,'EQUITY_BLUEPRINT_ABSENT',1;
DECLARE @blueprint_so bigint=(SELECT semantic_object_pk FROM model.blueprint WHERE blueprint_pk=@blueprint);
DECLARE @capability bigint,@capability_version bigint,@scenario_version bigint,@capability_digest binary(32);
SELECT @capability=c.capability_pk,@capability_version=ec.capability_version_pk,
 @scenario_version=cs.scenario_version_pk,@capability_digest=cv.definition_digest
...
IF (SELECT COUNT(*) FROM #operations)<>10 THROW 51000,'EQUITY_WORKING_EXECUTION_DIVERGED',1;
...
IF EXISTS (SELECT 1 FROM model.blueprint_version WHERE blueprint_pk=@blueprint AND definition_digest=@digest)
 BEGIN
  SELECT 'already_declared' AS result_set, @blueprint_id AS blueprint_id,
...
ELSE
 BEGIN
  ...
  INSERT model.semantic_object_definition(semantic_object_pk,object_kind,definition_digest,canonical_content_pk)
   VALUES(@blueprint_so,'BLUEPRINT',@digest,(SELECT content_object_pk FROM source.content_object WHERE content_digest=@digest));
  ...
  INSERT model.blueprint_version(blueprint_pk,semantic_object_pk,semantic_object_definition_pk,definition_digest,
   ...
   VALUES(@blueprint,@blueprint_so,@definition_pk,@digest,@capability,@capability_version,'canonical-circuit-blueprint.v1','CANDIDATE','BLUEPRINT',@definition_pk,N'');
  ...
  IF (SELECT COUNT(*) FROM model.provider_slot_operation WHERE _owner_definition_pk=@definition_pk)<>10
   THROW 51000,'EQUITY_PROVIDER_OPERATION_COVERAGE_INCOMPLETE',1;
 END;

SELECT 'blueprint_versions' AS result_set,bv.blueprint_version_pk,
 'sha256:'+LOWER(CONVERT(varchar(64),bv.definition_digest,2)) AS definition_digest,
 bv.source_disposition,bv.capability_version_pk
FROM model.blueprint_version bv WHERE bv.blueprint_pk=@blueprint ORDER BY bv.blueprint_version_pk;
SELECT 'slots' AS result_set,s.slot_id,n.projection_ordinal
FROM model.provider_slot s JOIN model.blueprint_node n ON n.blueprint_node_pk=s.owner_node_pk
WHERE s.blueprint_version_pk=(SELECT MAX(blueprint_version_pk) FROM model.blueprint_version WHERE blueprint_pk=@blueprint)
ORDER BY n.projection_ordinal;
...
COMMIT TRANSACTION;
-- Installed 2026-09-16: ten-slot blueprint for the fallback route; idempotent on (blueprint_pk, definition_digest).
