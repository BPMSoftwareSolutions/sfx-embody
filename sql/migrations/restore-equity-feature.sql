...
DECLARE @existingSod bigint = (SELECT semantic_object_definition_pk FROM model.semantic_object_definition WHERE semantic_object_pk=@featSo AND definition_digest=@ed);
DECLARE @action nvarchar(20) = CASE WHEN @existingSod IS NOT NULL THEN N'ALREADY_INSTALLED' ELSE N'MINTED' END;
DECLARE @sod bigint, @newFv bigint;
IF @action = N'MINTED'
BEGIN
...
BEGIN
  SET @sod = @existingSod;
  SELECT @newFv = MAX(feature_version_pk) FROM model.feature_version WHERE feature_pk=@featPk AND semantic_object_definition_pk=@existingSod;
END
...
SELECT '1_action' AS result_set, @action AS action, @newFv AS feature_version_pk, @capVer AS capability_version_pk, @scnVer AS scenario_version_pk;
