-- classify-equity-outcome-variants.sql
--
-- Declares the success/failure classification of
-- resolve-equity-market-price-evidence's outcome variants, so the kernel can
-- attach outcomeClassification to cell testimony
-- (cell-execution-testimony.v1) and the streamed display entry states the
-- declared result instead of only the mechanical completion:
--
--   scenario variants (model.outcome_variant rows of the capability's
--   selected scenario version):
--     EQUITY_MARKET_PRICE_EVIDENCE_RESOLVED     success
--     NATIVE_MARKET_PRICE_TESTIMONY_REJECTED    failure
--     EQUITY_MARKET_PRICE_PROVIDER_UNAVAILABLE  failure
--   operation variants (the execution authority definition's operations):
--     bind-equity-price-provider-credential     BOUND success;
--       CREDENTIAL_NOT_AVAILABLE / UNAUTHORIZED_REFERENCE / IDENTITY_MISMATCH
--       failure
--     observe-equity-price-exchange             completed success;
--       retained-non-success / rejected-endpoint / transport-failed /
--       rejected-credential / cancelled / oversized-response-rejected /
--       timed-out failure
--     normalize-equity-price-evidence           the three scenario dispositions
--
-- The execution authority is re-minted only to carry the operation variant
-- declarations; the scenario faces, ports, contracts and transformations are
-- untouched. The assembly functions are re-emitted so the declared
-- scenarioOutcomes (variantId + classification, absent when undeclared) travels
-- in the graph source and the semantic-graph document, where the kernel
-- compiles it into cell-level variant classifications.
--
-- Idempotent: the column and constraint are guarded, existing variant rows are
-- updated in place, an authority version carrying the same envelope digest is
-- reused, and the functions are CREATE OR ALTER. A re-run mints no duplicate.
--
...
DEALLOCATE @triggers;
GO
-- 1. The classification column. Declared variants may carry success/failure;
-- undeclared variants carry no classification and the kernel omits it. The DDL
-- is its own batch: later statements reference the column at compile time.
...
WHERE ec.estate_model_pk=@estate AND c.capability_id=@capability_id;
IF @capPk IS NULL THROW 51000,'EQUITY_CAPABILITY_NOT_FOUND',1;
DECLARE @scnPk bigint,@scnVer bigint;
SELECT TOP (1) @scnPk=cs.scenario_pk,@scnVer=cs.scenario_version_pk
FROM model.capability_scenario cs
WHERE cs.capability_version_pk=@capVer
ORDER BY cs.scenario_version_pk DESC;
IF @scnVer IS NULL THROW 51000,'EQUITY_SCENARIO_NOT_FOUND',1;

-- 2. The scenario outcome variants and their declared classification.
DECLARE @variant TABLE(variant_id nvarchar(400),classification varchar(16),ordinal int,PRIMARY KEY(variant_id));
INSERT @variant VALUES
 (N'EQUITY_MARKET_PRICE_EVIDENCE_RESOLVED','success',0),
 (N'NATIVE_MARKET_PRICE_TESTIMONY_REJECTED','failure',1),
 (N'EQUITY_MARKET_PRICE_PROVIDER_UNAVAILABLE','failure',2);
DECLARE @variant_owner bigint,@variant_terminal bit;
SELECT @variant_owner=so.semantic_object_definition_pk,@variant_terminal=CONVERT(bit,so.terminal)
FROM model.scenario_outcome so WHERE so.scenario_version_pk=@scnVer;
IF @variant_owner IS NULL THROW 51000,'EQUITY_SCENARIO_OUTCOME_NOT_FOUND',1;
DECLARE @variant_id nvarchar(400),@classification varchar(16),@ordinal int,@variant_cursor CURSOR;
SET @variant_cursor=CURSOR LOCAL FAST_FORWARD FOR SELECT variant_id,classification,ordinal FROM @variant;
OPEN @variant_cursor;
FETCH NEXT FROM @variant_cursor INTO @variant_id,@classification,@ordinal;
WHILE @@FETCH_STATUS=0 BEGIN
 IF EXISTS (SELECT 1 FROM model.outcome_variant WHERE scenario_version_pk=@scnVer AND variant_id=@variant_id)
  UPDATE model.outcome_variant SET classification=@classification,terminal=@variant_terminal
  WHERE scenario_version_pk=@scnVer AND variant_id=@variant_id;
 ELSE
  ...
  VALUES(@scnVer,@variant_id,@classification,@variant_terminal,@variant_owner,N'/variants/'+CONVERT(nvarchar(10),@ordinal));
 FETCH NEXT FROM @variant_cursor INTO @variant_id,@classification,@ordinal;
END;
CLOSE @variant_cursor;
DEALLOCATE @variant_cursor;

-- 3. The operation variant declarations. The authority is re-minted with the
-- declared variants only; every operation, port invocation and pointer carries
-- forward. An authority whose envelope already declares them is reused.
DECLARE @oldEaVer bigint,@eaPk bigint,@eaSo bigint,@profile nvarchar(400);
SELECT @oldEaVer=eav.execution_authority_version_pk,@eaPk=eav.execution_authority_pk,
       @eaSo=eav.semantic_object_pk,@profile=eav.authority_profile
...
WHERE se.scenario_version_pk=@scnVer;
IF @oldEaVer IS NULL THROW 51000,'EQUITY_EXECUTION_AUTHORITY_NOT_FOUND',1;
...
WHERE d.semantic_object_definition_pk=(SELECT semantic_object_definition_pk FROM model.execution_authority_version WHERE execution_authority_version_pk=@oldEaVer);
DECLARE @operation TABLE(port_id nvarchar(400),variants nvarchar(max),PRIMARY KEY(port_id));
INSERT @operation VALUES
 (N'bind-equity-price-provider-credential',
  N'["SUCCESS",{"variantId":"BOUND","classification":"success"},{"variantId":"CREDENTIAL_NOT_AVAILABLE","classification":"failure"},{"variantId":"UNAUTHORIZED_REFERENCE","classification":"failure"},{"variantId":"IDENTITY_MISMATCH","classification":"failure"}]'),
 ...
 (N'normalize-equity-price-evidence',
  N'["SUCCESS",{"variantId":"EQUITY_MARKET_PRICE_EVIDENCE_RESOLVED","classification":"success"},{"variantId":"NATIVE_MARKET_PRICE_TESTIMONY_REJECTED","classification":"failure"},{"variantId":"EQUITY_MARKET_PRICE_PROVIDER_UNAVAILABLE","classification":"failure"}]');
DECLARE @port_id nvarchar(400),@variants nvarchar(max),@opIndex int,@newEnv nvarchar(max)=@curEnv,@operation_cursor CURSOR;
SET @operation_cursor=CURSOR LOCAL FAST_FORWARD FOR SELECT port_id,variants FROM @operation;
OPEN @operation_cursor;
FETCH NEXT FROM @operation_cursor INTO @port_id,@variants;
WHILE @@FETCH_STATUS=0 BEGIN
 SET @opIndex=(SELECT TOP 1 CONVERT(int,[key]) FROM OPENJSON(@newEnv,'$.semantics.authority.operations')
  WHERE JSON_VALUE(value,'$.portId')=@port_id);
 IF @opIndex IS NULL THROW 51000,'EQUITY_OPERATION_NOT_DECLARED',1;
 SET @newEnv=JSON_MODIFY(@newEnv,'$.semantics.authority.operations['+CONVERT(varchar(10),@opIndex)+'].outcomeVariants',JSON_QUERY(@variants));
 FETCH NEXT FROM @operation_cursor INTO @port_id,@variants;
END;
...
DECLARE @existing bigint=(SELECT MAX(execution_authority_version_pk) FROM model.execution_authority_version
 WHERE execution_authority_pk=@eaPk AND definition_digest=@digest);
...
IF @existing IS NOT NULL SET @newEaVer=@existing;
ELSE BEGIN
 ...
 IF NOT EXISTS (SELECT 1 FROM model.estate_definition WHERE estate_model_pk=@estate AND semantic_object_definition_pk=@sod)
  INSERT model.estate_definition(estate_model_pk,semantic_object_definition_pk) VALUES(@estate,@sod);
 ...
 INSERT model.execution_operation(execution_authority_version_pk,operation_id,ordinal,operation_kind,_owner_definition_pk,_canonical_pointer)
  SELECT @newEaVer,operation_id,ordinal,operation_kind,@sod,_canonical_pointer
  FROM model.execution_operation WHERE execution_authority_version_pk=@oldEaVer;
 INSERT model.operation_port_invocation(execution_operation_pk,port_version_pk,operation_kind,_owner_definition_pk,_canonical_pointer)
  SELECT newOp.execution_operation_pk,opi.port_version_pk,opi.operation_kind,@sod,opi._canonical_pointer
  ...
  JOIN model.execution_operation oldOp ON oldOp.execution_operation_pk=opi.execution_operation_pk
   AND oldOp.execution_authority_version_pk=@oldEaVer
  JOIN model.execution_operation newOp ON newOp.execution_authority_version_pk=@newEaVer AND newOp.ordinal=oldOp.ordinal;
END;
UPDATE model.scenario_event SET execution_authority_version_pk=@newEaVer WHERE scenario_version_pk=@scnVer;

SELECT '1_classification_column' AS result_set,COL_LENGTH('model.outcome_variant','classification') AS classification_bytes,
 (SELECT COUNT(*) FROM sys.check_constraints WHERE name=N'CK_model_outcome_variant_classification') AS check_constraints;
SELECT '2_scenario_variants' AS result_set,s.scenario_id,ov.variant_id,ov.classification,ov.terminal
...
WHERE ov.scenario_version_pk=@scnVer ORDER BY ov.variant_id;
SELECT '3_operation_authority' AS result_set,@action AS action,@oldEaVer AS old_authority_version,@newEaVer AS new_authority_version,
 @digest AS definition_digest;
SELECT '4_operation_variants' AS result_set,eo.ordinal,JSON_VALUE(op.value,'$.portId') AS port_id,
 JSON_QUERY(op.value,'$.outcomeVariants') AS outcome_variants
FROM model.execution_operation eo
CROSS APPLY (SELECT CONVERT(nvarchar(max),CONVERT(varchar(max),co.content_bytes) COLLATE Latin1_General_100_BIN2_UTF8) AS envelope
 ...
 JOIN source.content_object co ON co.content_object_pk=d.canonical_content_pk
 WHERE eav.execution_authority_version_pk=@newEaVer) env
CROSS APPLY OPENJSON(env.envelope,'$.semantics.authority.operations') op
WHERE eo.execution_authority_version_pk=@newEaVer AND CONVERT(int,op.[key])=eo.ordinal
ORDER BY eo.ordinal;
GO
...
BEGIN
  ...
  SELECT b.estate_model_pk, b.capability_id, b.namespace_id, b.root_scenario_id,
    JSON_QUERY((SELECT b.capability_id AS capabilityId, b.root_scenario_id AS rootScenarioId,
      JSON_QUERY(b.scenarios) AS scenarios, JSON_QUERY(b.scenario_outcomes) AS scenarioOutcomes, JSON_QUERY(N'[]') AS transitions,
...
GO
DECLARE @estate bigint=(SELECT estate_model_pk FROM source.current_model WHERE singleton_id=1);
DECLARE @graph nvarchar(max)=(SELECT graph_source FROM analysis.capability_graph_source(N'resolve-equity-market-price-evidence',1,NULL));
SELECT '5_graph_source_scenario_outcomes' AS result_set,JSON_QUERY(@graph,'$.scenarioOutcomes') AS scenario_outcomes,
 (SELECT COUNT(*) FROM OPENJSON(JSON_QUERY(@graph,'$.executionAuthorities')) a
  CROSS APPLY OPENJSON(JSON_QUERY(a.value,'$.operations')) o
  WHERE JSON_QUERY(o.value,'$.outcomeVariants') IS NOT NULL) AS operations_with_variants,
 (SELECT COUNT(*) FROM OPENJSON(JSON_QUERY(@graph,'$.scenarioOutcomes')) s
  CROSS APPLY OPENJSON(JSON_QUERY(s.value,'$.variants')) v
  WHERE JSON_VALUE(v.value,'$.classification') IS NOT NULL) AS classified_scenario_variants;
SELECT '6_semantic_graph_document' AS result_set,
 JSON_QUERY(d.document,'$.scenarioOutcomes') AS scenario_outcomes
