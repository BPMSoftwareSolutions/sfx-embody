-- extend-observation-failure-testimony.sql
--
-- The observation channel now carries failure testimony, not only lifecycle
-- telemetry and admitted shapes: when admission, compilation or a cell
-- execution fails, the lane publishes its own declared event carrying the
-- failure code, the bounded failure message and the cell/edge identity the
-- failure belongs to. The model response shape declared by
-- extend-observation-telemetry-shapes.sql stays declared; the connector path
-- now publishes it as its own event.
--
-- The allowlist remains declared authority (read-observation-telemetry-authority);
-- this migration extends the declared object fields rather than naming a shape
-- member in code:
--
--   observationFields.failureCode      the declared failure identity
--   observationFields.failureMessage   the bounded failure message
--   objectFields.shapes.modelResponse  the normalized model response shape
--   objectFields.payloadByteBound     4096; a message or payload over it is
--                                     clipped or referenced by digest
--   objectFields.eventByteBudget      65536 per observation
--   objectFields.invocationByteBudget 524288 per invocation
--
-- Credential material, authorization headers, vault material and environment
-- values are still not among the declared fields; the denied members are
-- removed from any payload before it is published.
--
-- Idempotent: the capability document's digest is the installed-document gate,
-- and the contracts are content-addressed.
--
-- Default: ROLLBACK. The commit variant (extend-observation-failure-testimony.commit.sql)
-- replaces the final ROLLBACK TRANSACTION; with COMMIT TRANSACTION; after the
...
DECLARE @document nvarchar(max) = N'{
 ...
 "meaning":{"intent":"read the declared observation-channel telemetry authority","outcome":"the caller observes the declared scalar fields, the bounded shape fields, the failure-testimony fields, the entry fields and the bounded object fields the observation channel may publish"},
 ...
 "scenarios":[{
  ...
  "then":"the reading names the scalar testimony fields, the failure-testimony fields, the display Entry fields, the declared shape fields with their payload bound and budgets, and the bounded provider-evidence fields the observation channel may publish",
...
-- names, the failure-field names, the denied-member count and the bound/budget
-- facts so a truncated or misplaced list is visible.
...
SELECT '3_authority_self_test' AS result_set,
 ...
 (SELECT COUNT(*) FROM OPENJSON(JSON_QUERY(value,'$.objectFields.shapes.modelResponse'))) AS model_response_members,
...
-- Dry run: the declared extension is proved above and rolled back. The commit
-- variant installs after the from-transaction preflight passes.
