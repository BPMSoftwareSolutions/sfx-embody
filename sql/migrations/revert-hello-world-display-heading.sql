-- revert-hello-world-display-heading.sql
--
-- Reverts change-hello-world-display-heading.sql so the estate is left on the
-- U1 declaration: the display heading template returns to "Scenario
-- {scenarioId}". The reverted definition is a new definition row (the original
-- digest still exists and cannot be re-selected as latest), so a `revertedAt`
-- marker distinguishes it; the projected transformation is id + expression, and
-- the expression equals the U1 declaration exactly.
--
-- Idempotent: a second run finds the reverted marker and re-declares nothing.
--
...
IF JSON_VALUE(@semantics,'$.revertedAt') IS NOT NULL
 SELECT '1_heading_revert' AS result_set, N'ALREADY_REVERTED' AS action, @heading AS heading_template;
ELSE BEGIN
 SET @semantics=JSON_MODIFY(@semantics,'$.expression.bindings.blocks.from.items[0].fields.text.template',N'Scenario {scenarioId}');
 SET @semantics=JSON_MODIFY(@semantics,'$.revertedAt',CONVERT(nvarchar(40),SYSUTCDATETIME(),126));
 ...
 END
 SELECT '1_heading_revert' AS result_set, N'REVERTED' AS action, @heading AS heading_before,
