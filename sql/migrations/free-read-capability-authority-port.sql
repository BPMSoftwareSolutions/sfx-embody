-- Free `read-capability-authority` from the deleted resolver
-- `authority-read-provider.readCapabilityAuthority`. The Port is re-declared as a
-- declared data read over the estate's own view; no configuration.providerId or
-- estateProvider remains. configuration.defaultTarget is retained because the
-- frontdoor reads it to resolve the execution delivery target.
--
...
DECLARE @ns nvarchar(400)=N'sidefx:capability:read-capability-authority';
DECLARE @port nvarchar(400)=N'read-capability-authority-port';
...
SELECT 'freed_port' AS result_set, d.declared_id,
 ...
 JSON_VALUE(d.definition_json,'$.semantics.configuration.defaultTarget') AS default_target,
