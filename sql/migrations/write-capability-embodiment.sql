-- write-capability-embodiment.sql
--
-- Declares `write-capability-embodiment`: the write step of the embody loop. It
-- consumes an embodiment plan, reproduces it from the same authority, writes it
-- beneath the declared output root, and refuses if the written artifact diverges
-- from the plan's digests.
--
...
EXEC model.scaffold_estate_provider_capability
  @capability_id=N'write-capability-embodiment',
  @provider_module=N'src/resolvers/node/embodiment-write-provider.mjs',
  @provider_export=N'writeCapabilityEmbodiment',
  @input_contract=N'capability-embodiment-plan.v1', @input_schema=@plan,
  @outcome_contract=N'capability-embodiment-materialization.v1', @outcome_schema=@materialization,
  @description=N'Write a capability embodiment plan to an authorized root';
