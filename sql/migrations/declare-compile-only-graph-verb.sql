-- declare-compile-only-graph-verb.sql
--
-- D4 (observation-altitudes implementation plan, 2026-09-23): the C#-only
-- `--plan-only` flag is replaced by a declared compile verb in the kernel's
-- command operations authority (`sda-kernel-command-operations.v1`). The verb
-- is rows, not a flag:
--
...
--
-- The authority is content-addressed re-declared: the selected bytes are read,
-- the operation row is set, the new bytes are retained in source.content_object
-- and the AUTHORITY semantic definition is re-declared under the same id with
-- the new content digest.
--
-- Idempotent: a second run finds the row selected, sets nothing and re-declares
-- nothing; the preflight and proof result sets are identical.
--
-- Default: ROLLBACK. Replace the final ROLLBACK with COMMIT to install after
-- the dry run (see declare-compile-only-graph-verb.commit.sql).
SET NOCOUNT ON;
SET XACT_ABORT ON;
BEGIN TRANSACTION;
...
-- To install, use declare-compile-only-graph-verb.commit.sql (COMMIT).
