...
--
-- Declares model.install_provider_binding_change: the declared install mechanic
-- for one provider binding change on one capability. The change document is the
-- same document `sfx provider set` authors and preflights against
-- (provider-binding-change.v1). This procedure is the install half: it appends
-- one provider route (request builder, credential binding, exchange request,
-- exchange observation, normalization/selection) to the capability's execution
-- authority, declares the route's ports, mints the endpoint authority content
-- address from the declared endpoint record, re-declares the authority version,
-- and re-declares the provider-slot blueprint for the new operation count.
--
-- The route's normalization expression is assembled from the document's declared
-- `mapping` (each canonical field names the native path that answers it, or a
-- `request:` path). Nothing here fabricates a value: a field with no declared
-- native source is refused by the authoring read before this procedure runs.
--
-- Adding a provider route is data: one document, one call, no code edit. The
-- route is appended to whatever the capability currently declares, so the chain
-- is N-deep (primary, then every fallback in install order).
--
-- Idempotent: a route whose binding port already exists in the selected
-- authority is reported already_installed and writes nothing.
--
...
GO
BEGIN TRANSACTION;
-- The wrapper migration calls the procedure with the finance15 change document;
-- the dedicated wrapper file is the installed change. This migration only
-- declares (or re-declares) the procedure.
COMMIT TRANSACTION;
-- Installed 2026-09-19: the declared provider binding change install mechanic.
