-- Declared capability for the estate loader `src/load-memory-scenario.mjs`:
-- identity + contracts + a root Scenario whose Port selects the declared provider.
SET NOCOUNT ON;
SET XACT_ABORT ON;
BEGIN TRANSACTION;
...
DECLARE @providerId nvarchar(400)=N'load-memory-scenario.loadMemoryScenario';
...
EXEC model.put_semantic_definition 'PROVIDER',N'sidefx:providers',@providerId,
 N'{"module":"src/load-memory-scenario.mjs","export":"loadMemoryScenario","declarationProfile":"sda-estate-provider-implementation.v1"}',
...
EXEC model.scaffold_capability @capability_id=N'load-memory-scenario', @on_exists=N'REPLACE';
EXEC model.declare_contract @id=N'load-memory-scenario-request.v1', @schema=N'{"type":"object","additionalProperties":true}';
EXEC model.declare_contract @id=N'load-memory-scenario-result.v1', @schema=N'{"type":"object","additionalProperties":true}';
GO
EXEC model.declare_scenario
 @capability_id=N'load-memory-scenario',
 @scenario=N'{"scenarioId":"load-memory-scenario","name":"Load one planned embodiment body in memory","inputId":"load-memory-scenario-request","inputContract":"load-memory-scenario-request.v1","eventId":"load-memory-scenario-requested","eventAuthority":"load-memory-scenario.v1","outcomeId":"load-memory-scenario-result","outcomeContract":"load-memory-scenario-result.v1","terminal":true,"root":true,"given":"one planned embodiment (carried bytes)","when":"the body is loaded in memory under the read boundary","then":"an executable runtime is returned or the load is held"}',
 @operations=N'[{"operationId":"load-memory-scenario.0","kind":"invoke-port","portId":"load-memory-scenario-port"}]',
 @port_bindings=N'[{"portId":"load-memory-scenario-port","platformCapabilityId":"sda-embodiment-plan-port.v1","configuration":{"providerId":"load-memory-scenario.loadMemoryScenario"}}]';
GO
SELECT 'load_memory_scenario_capability' AS result_set, c.capability_id,
...
WHERE c.capability_id=N'load-memory-scenario';
COMMIT TRANSACTION;
