-- LANE B TIER 1 (contract admission): free the Ports that bind
-- `contract-admission-provider.admitDeclaredContract` by clearing
-- configuration.providerId. Each Port keeps its platform binding
-- (`platformCapabilityId`, `admissionProvider`, admission expressions) and so
-- resolves to the declared contract-admission mechanic. Then delete the
-- now-unreferenced PROVIDER row. Idempotent.
--
...
DECLARE @provider_id nvarchar(400)=N'contract-admission-provider.admitDeclaredContract' COLLATE Latin1_General_100_BIN2;
...
INSERT @targets (namespace_id, port_id) VALUES
 (N'sidefx:capability:admit-declared-contract', N'admit-declared-contract-port'),
 (N'sidefx:capability:contract-admission-provider', N'contract-admission-provider-port'),
 (N'sidefx:capability:execute-declared-capability', N'admit-execution-input'),
 (N'sidefx:capability:execute-declared-capability', N'admit-execution-outcome');
...
SELECT 'freed_port' AS result_set, d.declared_id,
 ...
 JSON_VALUE(d.definition_json,'$.semantics.configuration.admissionProvider.providerId') AS admission_provider
FROM analysis.v_selected_semantic_definition d
WHERE d.estate_model_pk=@estate AND d.object_kind='PORT'
