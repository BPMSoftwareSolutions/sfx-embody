...
BEGIN
  ...
  SELECT @sod = semantic_object_definition_pk FROM model.execution_authority_version WHERE execution_authority_version_pk=@existing;
END
...
SELECT '2_new_equity_root' AS result_set, eo.ordinal, eo.operation_kind, p.port_id
FROM model.execution_operation eo
...
WHERE eo.execution_authority_version_pk=@newEaVer
ORDER BY eo.ordinal;
SELECT '3_root_event' AS result_set, se.scenario_version_pk, se.execution_authority_version_pk
