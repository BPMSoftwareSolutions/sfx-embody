-- Rebind the Ports whose platform capability the platform implements to the
-- declared platform provider, so configuration.providerId selects the platform
-- provider and the helper view projects its declared module/export.
SET NOCOUNT ON;
SET XACT_ABORT ON;
BEGIN TRANSACTION;
...
JOIN (VALUES
 (N'sidefx:capability:execute-declared-capability', N'admit-execution-input',  N'ScenarioKernel.NodePlatform.Schema.JsonSchemaContractAdmission'),
 (N'sidefx:capability:execute-declared-capability', N'admit-execution-outcome',N'ScenarioKernel.NodePlatform.Schema.JsonSchemaContractAdmission'),
 (N'sidefx:capability:project-consumer-execution-embodiment-plan', N'derive-target-neutral-execution-plan', N'ScenarioKernel.NodePlatform.Execution.SemanticExecutionGraphCompilation')
) m(namespace_id, port_id, provider_id)
 ON m.namespace_id=d.namespace_id COLLATE Latin1_General_100_BIN2 AND m.port_id=d.declared_id COLLATE Latin1_General_100_BIN2
...
CLOSE ports;
DEALLOCATE ports;
SELECT 'rebound_ports' AS result_set, (SELECT COUNT(*) FROM @targets) AS rebound,
 (SELECT COUNT(*) FROM analysis.v_selected_semantic_definition d WHERE d.estate_model_pk=@estate AND d.object_kind='PORT'
   AND JSON_VALUE(d.definition_json,'$.semantics.configuration.providerId') IN
     (N'ScenarioKernel.NodePlatform.Schema.JsonSchemaContractAdmission',N'ScenarioKernel.NodePlatform.Execution.SemanticExecutionGraphCompilation')) AS on_platform_provider;
COMMIT TRANSACTION;
