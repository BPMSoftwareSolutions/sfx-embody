-- Bound invocation reads before assembling documents. Existing views stay available.
-- Declaration rows and graph_source share one local assembly, never a persistent cache.
-- The functions participate in the existing analysis SQL-definition coherence check.
-- Port and transformation sets follow semantic-object order; inner arrays and expression members are untouched.
-- SELECT * FROM analysis.capability_graph_source(N'say-hello-world', 0, NULL);
-- Pass 1 to also return documents as JSON rows (source_path, entry_id, document).
SET NOCOUNT ON;
SET XACT_ABORT ON;
BEGIN TRANSACTION;
...
-- No model/source rows change here. Restore the guard DDL, including all metadata,
-- immediately; a later installation must not remove or enable/disable any guard.
SAVE TRANSACTION bound_invocation_guards;
DECLARE @trigger_name nvarchar(517), @triggers CURSOR;
SET @triggers = CURSOR LOCAL FAST_FORWARD FOR
  SELECT trigger_name FROM @guards;
OPEN @triggers;
...
CLOSE @triggers;
DEALLOCATE @triggers;
ROLLBACK TRANSACTION bound_invocation_guards;
IF EXISTS (
...
) THROW 51000, 'BOUND_INVOCATION_GUARDS_NOT_RESTORED', 1;
...
BEGIN
  ...
  -- Ports select the newest included definition; provider resolution keeps its own rule.
  ...
  UPDATE p SET semantics =
    CASE WHEN JSON_VALUE(decoded.definition_json, '$.semantics.configuration.providerId') IS NULL
      ...
      ELSE JSON_MODIFY(JSON_QUERY(decoded.definition_json, '$.semantics'), '$.configuration.estateProvider',
        JSON_QUERY(analysis.fn_estate_provider_semantics(JSON_VALUE(decoded.definition_json, '$.semantics.configuration.providerId')))) END
  FROM @ports p
  CROSS APPLY (
  ...
  SELECT @estate_model_pk, @capability_id, @prefix + N'semantic-transformation.authority.json', N'semantic-transformation.authority.json',
    ...
    + ISNULL((SELECT STRING_AGG(JSON_QUERY(envelope, '$.semantics'), N',')
      WITHIN GROUP (ORDER BY semantic_object_pk) FROM @transformations), N'') + N']}'
  UNION ALL
  ...
  SELECT @estate_model_pk, @capability_id, @prefix + N'consumer-workspace.authority.json', N'consumer-workspace.authority.json',
    N'{"workspaceType":"consumer-workspace-authority.v1","consumerId":"' + @capability_id + N'","projectionTargets":["node"],'
