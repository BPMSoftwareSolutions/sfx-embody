...
export async function readAuthority(databaseRoot, selection, { retainObjects = true } = {}) {
  const { query } = await import(pathToFileURL(path.join(databaseRoot, 'src/query/run.mjs')));
  const read = async name => query(await fs.readFile(path.join(databaseRoot, 'sql/diagnostics', name), 'utf8'), { input: selection, rowLimit: 100000, retainObjects });
