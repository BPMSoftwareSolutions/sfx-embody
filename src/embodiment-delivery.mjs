...
// The embodiment materialization delivery is a distinct governed effect boundary.
// Unlike the read-only database-memory delivery, it is authorized to write the
// embodiment root; it is not subject to the memory-only storage proof control,
// which exists to prove that invocation reads no retained cache. Everything else
// is the same transport as the read-only delivery.
try {
  const chunks = [];
  let size = 0;
  ...
  const processEvidence = { fsWriteAllowed: true, authorizedWriteRoot: 'embodiments', expandedBodyReadAllowed: false, databaseCacheReadAllowed: false };
