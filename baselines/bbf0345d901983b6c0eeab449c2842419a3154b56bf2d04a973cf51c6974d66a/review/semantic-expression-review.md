The current bodies execute the retained fixtures, but their source does not adequately express the Capability's meaning. The principal defect is in the candidate embodiment resolver: it materializes an expression-evaluation object graph and exposes construction order as application vocabulary. Renaming numbered variables alone would leave that architecture in place.
