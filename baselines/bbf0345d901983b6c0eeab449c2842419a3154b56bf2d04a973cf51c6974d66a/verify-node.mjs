...
export async function verifyNode(base, sdaRoot, outputRoot) {
  ...
  for (const [scenarioId, target] of targetBases) {
    ...
    const nativeAst = ts.createSourceFile('native.mjs', nativeSource, ts.ScriptTarget.Latest, true);
    const evaluator = nativeAst.statements.find(s => ts.isFunctionDeclaration(s) && s.name?.text === authority.nativeBinding.implementation_export);
    const clauses = evaluator.body.statements.find(ts.isSwitchStatement).caseBlock.clauses;
    const mechanicFile = path.join(target, 'body/providers/mechanics.mjs');
    const mechanicAst = ts.createSourceFile(mechanicFile, await fs.readFile(mechanicFile, 'utf8'), ts.ScriptTarget.Latest, true);
    const lineage = await readJson(path.join(target, 'evidence/mechanic-lineage.json'));
    const nativeChecks = [];
    for (const operation of new Set(lineage.flatMap(l => l.nodes.map(n => n.operation)))) {
      const declaration = clauses.find(c => ts.isCaseClause(c) && c.expression.text === operation);
      const className = operation.split(/[^A-Za-z0-9]+/).filter(Boolean).map(x => x[0].toUpperCase() + x.slice(1)).join('') + 'Mechanic';
      const physical = mechanicAst.statements.find(s => ts.isClassDeclaration(s) && s.name.text === className);
      const method = physical?.members.find(m => ts.isMethodDeclaration(m) && m.name.text === 'execute');
      const normalize = (statements, ast) => statements.map(s => printer.printNode(ts.EmitHint.Unspecified, s, ast)).join('\n');
      const passed = Boolean(method) && normalize(declaration.statements, nativeAst) === normalize(method.body.statements, mechanicAst);
      nativeChecks.push({ operation, className, passed });
      assert.ok(passed, 'native body fidelity: ' + operation);
    }
    mechanicClassCount += nativeChecks.length;
    ...
    const reveal = { capabilityId: receipt.capabilityId, scenarioId, target: 'node', method: 'TypeScript AST inspection of materialized files; native method statements compared with selected provider',
      files, nativeSource: { path: authority.nativeBinding.implementation_id, digest: hash(nativeSource) },
      contractTypeCheck: { files: contractFiles.length, diagnostics: diagnostics.length }, nativeBodyChecks: nativeChecks, observedClasses };
    ...
    revelations.set(scenarioId, { target, reveal, receipt, nativeChecks });
  }
  ...
  for (const [scenarioId, { target, receipt, reveal, nativeChecks }] of revelations) {
  ...
  return { capabilityId: rootReceipt.capabilityId, scenarioId: rootReceipt.scenarioId, fixtures: results.length, passed: results.filter(r => r.passed).length,
    ...
    kernelObservations: results.reduce((n, r) => n + r.observations.length, 0), mechanicInvocations: results.reduce((n, r) => n + r.mechanicObservations.length, 0), negativeChecks: nestedFailure ? 2 : 1 };
}
