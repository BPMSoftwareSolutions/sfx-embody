...
export class AdmitCanonicalCircuitBlueprintScenario {
    ...
    async perform(input, context) {
        const state1 = await this.dependencies["admit-canonical-circuit-blueprint-port"].execute(input, context.rootInput);
        const state2 = await this.dependencies["require-blueprint-conformance-evidence"].invoke(state1, context, 1);
        const state3 = await this.dependencies["require-blueprint-geometry-proof"].invoke(state2, context, 2);
        const state4 = await this.dependencies["require-current-approved-review-receipt"].invoke(state3, context, 3);
        const state5 = await this.dependencies["emit-immutable-blueprint-authority"].invoke(state4, context, 4);
        return state5;
    }
