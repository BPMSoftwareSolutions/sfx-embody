...
export class AdaptJobMarketIntelligenceEvidenceScenario {
    ...
    async perform(input, context) {
        const state1 = await this.dependencies["adapt-job-market-intelligence-evidence-port"].execute(input, context.rootInput);
        const state2 = await this.dependencies["verify-jmi-record-binding"].invoke(state1, context, 1);
        const state3 = await this.dependencies["verify-jmi-type-admission"].invoke(state2, context, 2);
        const state4 = await this.dependencies["bind-jmi-adapter-receipt"].invoke(state3, context, 3);
        return state4;
    }
