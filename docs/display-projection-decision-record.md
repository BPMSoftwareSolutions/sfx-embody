...
| **D5** | Provider/physical empty altitudes: localize before deciding data vs pipeline | Missing/conflicting evidence (compiler/scheduler support exists; production yields zero cells); rubric §3 investigate | The granular-tracing claim in the demo cannot be made honestly | Bounded read-only probe; outcome decides a declared-data migration or an SDA request | 3/0 | **Needed now as a bounded investigation** (no commitment until evidence) |
...
D6a additive `--json`, D7 streamed status fields inside U4, and the bounded D5
provider/physical investigation. D2a is needed now and already filed as SDA R1.
