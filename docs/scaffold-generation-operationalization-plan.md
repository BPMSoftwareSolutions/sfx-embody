...
| Versioned mechanics, providers, profiles and bindings in SQL | Database catalog (`sidefx-database` migration catalog, **retired** — named, not linked) | Extend exact retrieval/projection only where data is missing; avoid a parallel mechanic catalog. Existing tables alone do not establish complete semantic coverage. |
...
- Map each required field to existing authoritative bytes, normalized mechanic/port/provider/profile/binding rows and qualification evidence. Extend the catalog, schema and views only for demonstrated projection gaps. (Those modules lived in the retired `sidefx-database` tree; they are named, not linked, and any equivalent is now a migration under `sql/`.)
...
- Use the supported registration path for authored capsule bytes, with validation. (The era's registration module lived in the retired `sidefx-database` tree — named, not linked; registration is now a migration under `sql/`.) Coordinate canonical-feature bindings with the [feature migration](canonical-feature-migration-gap.md). Prove the pilot's exact binding before its database invocation; the full estate backfill remains that migration's work.
