...
| CV-D1 structural acceptance | **rejected as UID; re-specified as declared authority** | `c01e777` added `scripts/verify-circuit-structure.mjs` with the classification in code — exactly the UID we do not create. The script and its `verify:circuit` entry are removed; the verdict moves into the declared read (`attestation` section, see the CV-D1 row above). The branch-aware rule the script proved (observed→planned exact; unobserved planned cells only off the taken path) is the declared rule's specification, and the live receipt it produced remains as the expected-value evidence |
