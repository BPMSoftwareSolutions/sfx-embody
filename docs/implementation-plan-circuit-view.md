...
| CV-D1 | D — verification | Structural acceptance: planned↔observed bijection per exercised capability, as a declared read over the same inputs (or extending `verify:timing`); combined verdict with the time test | `scripts/verify-timing-coherence.mjs` or a sibling script | A1, B1 | agent lane, equity, compose, model lane: every planned cell observed, every observed cell planned; verdict line names both halves |
