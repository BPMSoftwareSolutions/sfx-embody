...
| Perf comparison | `SDA:conformance/projected-performance/projected-performance.mjs` reports whole-invocation median/p95 and cross-target parity for node/python/csharp with per-cell timing read from testimony (estate U5c deleted the `scripts/` copy); `SDA:conformance/projected-testimony/verify-projected-testimony.mjs` is the closure probe |
...
1. **Per-cell timing and testimony parity in generated bodies (F1, F2) — filed.**
   ...
   closure probe (estate U5c homed it at
   `SDA:conformance/projected-testimony/verify-projected-testimony.mjs`) reported `TESTIMONY OPEN`
...
| 12 | Reader operations must execute under the invocation path's host bindings (`ReadQuery`), and a failed read must carry its cause (F13) | **Landed** — SDA `925587b` (shared `CreateReadQueryBinding`, cause-bearing failures, ReaderOperationConformance) + `3edf004` (boolean `formats`, specific refusals, circuit end to end); installed artifact `f3ae79b1…`; estate re-point `ee8e0b4` (`sql/migrations/repoint-circuit-reader-to-live-ancestor-walk.sql`); live 2026-09-19: `list`, `reveal`, `circuit`, `observe --format circuit` all complete |

## 5. Findings register

| # | Finding | Class | Evidence |
|---|---|---|---|
| F1 | Projected bodies record cell/edge/resolver testimony with digests and logical order but **no timing fields** in node, python or csharp | **Landed** — SDA `a696fdd`/`0bff62b`: timing fields on every cell/edge/pattern record; the closure probe is homed at `SDA:conformance/projected-testimony/verify-projected-testimony.mjs` (estate U5c) | `docs/sda-change-request-projected-testimony.md`; `evidence/vault-20260916/u5c/projected-testimony.conformance.json` |
| F2 | Projected testimony is not cross-language parity: python `observedPathDigest: null`, csharp omits it and `resolverTestimony`, cell-id sets differ (node 8 / python 111 / csharp 5; 70 python ids semantic-id normalized) | **Partially landed** — SDA `a696fdd`/`0bff62b`: every target emits `observedPathDigest` + `resolverTestimony`, equal digests asserted on the shared chaining fixtures; remaining on the equity fixture: python records semantic-normalized `cellId`s and the walkers record different granularities (asserted per target) | `evidence/vault-20260916/u5c/projected-testimony.conformance.json` |
...
| F5 | The database copy is installed as an 8.5 MB migration, so the bodies live in git twice (files + rows) | Revisit trigger: a persistence path replaces the generated migration; the generator is **deleted** (estate U5c — materialization is eliminated, SDA M1 reverted the database target) | `sql/migrations/publish-projected-bodies-*.sql` (historical install); `evidence/vault-20260916/u5c/receipt.json` |
...
| F11 | **Invisible execution authority (IEA)** — the temporal face of UID: streamed invocations do not validate the time between events, so work with no cell (spawns, boot logic, side channels) or a hidden altitude reads as unattributed wall time; the first receipt showed a ~5 s hole at story altitude that traces to declared cells (connector 6.17 s, vault credential 849 ms) | **Landed** — three units: terminal gap display (CLI `d68a4f3`), declared timing reading `read-invocation-timing` (estate `612609d`), acceptance `npm run verify:timing` (agent lane 715/715 windows closed, unaccounted 2 ms; equity 179/179, 1 ms; verdict TIMING-COHERENT) | `docs/invisible-execution-authority.md` |
| F12 | **Raw non-ASCII literals corrupt on the rows→runtime path**: content bytes are decoded as `CONVERT(nvarchar, CONVERT(varchar, content_bytes))` under the database default collation (`SQL_Latin1_General_CP1_CI_AS`), which misreads UTF-8 as cp1252, so a declared glyph like `✓` reaches the runtime as mojibake; JSON `\uXXXX` escapes are the one working form (they decode after parsing). Discovered declaring the circuit presentation glyph set (`6e90603`) | Finding — decode defect in the estate-wide read path; escapes are the interim working form | `sql/migrations/declare-circuit-presentation.sql` header; `docs/implementation-plan-circuit-view.md` §UID audit |
| F13 | **Reader operations fail on the installed C# kernel**: `capability list`, `find`, `reveal`, `catalogue`, `circuit`, `media artifact` all fail with the collapsed `DECLARED_READ_FAILED` because `CommandCarrier.ExecuteReaderOperationAsync` builds a `CarrierContext` without host bindings (no `ReadQuery`); the same declared reads complete through `capability invoke` (list-capabilities returns the full catalogue), and the failure cause is hidden by the generic fallback at `CommandCarrier.cs:199-203` | **Landed** — SDA `925587b` (reader host bindings, failed read's own message, ReaderOperationConformance) and `3edf004` (boolean formats, specific refusals, circuit end to end); live 2026-09-19 on the re-mediated kernel: `capability list` returns the catalogue, `reveal` the meaning document, `circuit` the live ancestor walk `circuit-view.v1`, `observe resolve-equity-market-price-evidence --input AVGO --format circuit` renders the box stream with live evidence | live 2026-09-19: reader ops failed via CLI and via `KernelEntry.exe` directly; `CommandCarrier.cs:192-194` vs `InvocationBoot.cs:205-230` vs `command_carrier.py:210`; `docs/sda-change-request-reader-operation-host-bindings.md` |
