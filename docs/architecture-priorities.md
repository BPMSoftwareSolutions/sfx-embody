...
| F11 | **Invisible execution authority (IEA)** — the temporal face of UID: streamed invocations do not validate the time between events, so work with no cell (spawns, boot logic, side channels) or a hidden altitude reads as unattributed wall time; the first receipt showed a ~5 s hole at story altitude that traces to declared cells (connector 6.17 s, vault credential 849 ms) | Concept named + detection rule recorded; timing fields already declared in all six languages | `docs/invisible-execution-authority.md` |
