...
| 10 | E | Declare `narrate-*`/`diagram-*` as templates/transformations (or request a presentation mechanic if not expressible) | reveal renders from data | done-with-disposition: no estate capability — reading/selection are declared rows, rendering is terminal (`sidefx-cli/src/render.mjs`) plus the SDA presentation seam; revisit on a non-CLI consumer or an inexpressible template ([narration-projection-disposition.md](narration-projection-disposition.md)) |
