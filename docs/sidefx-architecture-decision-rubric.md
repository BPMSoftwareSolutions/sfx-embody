...
This section applies the rubric to one bounded work order — [SQL → CLI: scaffold Hello World](sql-cli-work-order-001.md) — and to the artifact `sql/migrations/scaffold-hello-world.sql`. Its purpose is to decide which records, bindings, generations and lifecycle steps are **necessary to close that loop**, and which are promotion machinery the loop does not require.
