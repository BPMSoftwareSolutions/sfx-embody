...
-- `resolve-sidefx-eligible-providers` can. Run each block under the restricted
-- reader, which supplies @estate_model_pk:
--
--   cd C:\lab\sidefx-database
--   npm run query -- --file <this file>            (or paste a single block)
--
...
--    not covered, so a zero here is "no open array in its own capsule schemas".
---------------------------------------------------------------------------
DECLARE @snapshot bigint;
SELECT @snapshot = estate_snapshot_pk FROM source.estate_model WHERE estate_model_pk = @estate_model_pk;
...
WITH capsule AS (
  ...
  FROM source.source_appearance a
  JOIN source.content_object c ON c.content_object_pk = a.content_object_pk
  WHERE a.estate_snapshot_pk = @snapshot AND a.source_class = 'MANAGED_CAPSULE'
...
---------------------------------------------------------------------------
-- 4. Blast radius. Same walk, grouped, no capability filter. Drop the two
--    IN-list lines from query 3 and group -- shown here as its own statement.
---------------------------------------------------------------------------
DECLARE @s bigint;
SELECT @s = estate_snapshot_pk FROM source.estate_model WHERE estate_model_pk = @estate_model_pk;
...
WITH capsule AS (
  ...
  FROM source.source_appearance a
  JOIN source.content_object c ON c.content_object_pk = a.content_object_pk
  WHERE a.estate_snapshot_pk = @s AND a.source_class = 'MANAGED_CAPSULE'
    AND a.entry_id = 'capability.authority.json'
),
schemas AS (
  SELECT DISTINCT k.capability_id, a.entry_id,
