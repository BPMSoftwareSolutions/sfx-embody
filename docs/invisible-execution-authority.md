...
| Timing-coherence acceptance | estate `612609d`; extended 2026-09-17; homed on the kernel ground 2026-09-18 (U5c) | `npm run verify:timing` (now `SDA:languages/typescript/src/kernel/bootstrap/timing-coherence.mjs`) still covers the four live cases: agent lane **715 windows closed** / unaccounted **3 ms**; equity **179** / **3 ms**; compose `compose-resolve-equity-market-price-evidence` **181** / **1 ms**; model lane `obtain-governed-model-response` **460** / **0 ms**; verdict `TIMING-COHERENT (every residual zero or named)`; receipt `evidence/vault-20260916/u5c/timing-coherence.receipt.json` |
| Projected-target acceptance | **landed 2026-09-18** (U5c) | The kernel emitters now stamp `startedAt`/`completedAt`/`durationMilliseconds` on every projected testimony record and emit `observedPathDigest` + `resolverTestimony` on every target (SDA `a696fdd`, `0bff62b`; the cross-target emitter test asserts equal digests on the shared chaining fixtures). The conformance probe is homed at `SDA:conformance/projected-testimony/verify-projected-testimony.mjs`; per-cell timing is available on a regenerated projection (`evidence/vault-20260916/u5c/projected-performance.report.json`). Remaining on the equity F6 fixture: python testimony still carries semantic-normalized `cellId`s (not canonical) and the walkers record different cell granularities, so cross-target cell-set/digest equality is asserted on the shared chaining fixtures, not on this fixture |
