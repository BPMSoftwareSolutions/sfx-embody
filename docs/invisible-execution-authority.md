...
| Timing-coherence acceptance | estate `612609d`; extended 2026-09-17 | `npm run verify:timing` now covers four live cases: agent lane **715 windows closed** / unaccounted **2 ms**; equity **179** / **1 ms**; compose `compose-resolve-equity-market-price-evidence` **181** / **0 ms**; model lane `obtain-governed-model-response` **460** / **1 ms**; verdict `TIMING-COHERENT (every residual zero or named)`; receipt `evidence/vault-20260916/iea/timing-coherence.receipt.json` |
| Projected-target acceptance | **waits on F1** | The projected node/python/csharp sources emit no `durationMilliseconds`/`startedAt` in their testimony (grep over the projected bodies is empty), so the gap test has nothing to close against. The emitter-timing request is already filed ([sda-change-request-projected-testimony.md](sda-change-request-projected-testimony.md); register request 7); the acceptance extends to projected targets the moment projected testimony carries timing |
