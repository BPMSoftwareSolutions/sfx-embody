...
// invocation delivery, and the connection string now resolves from the
// declared credential vault: no environment variable is read or copied.
test('the projection boot resolves at the connect boundary and never copies the string into process.env', async () => {
  ...
  assert.equal(source.includes('connectionEnvironmentVariable'), false);
  assert.match(source, /connectionName: connectionCredentialReference/);
});
